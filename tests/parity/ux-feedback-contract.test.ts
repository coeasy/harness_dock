...
describe('desktop interaction feedback contract', () => {
  ...
  it('acknowledges supervised quit before waiting for managed processes', () => {
    ...
    const overlay = supervisor.indexOf('show_primary_lifecycle_overlay(')
    const drain = supervisor.indexOf('wait_for_managed_processes(handle.clone()).await')
    expect(overlay).toBeGreaterThanOrEqual(0)
    expect(drain).toBeGreaterThan(overlay)
    expect(supervisor).toContain('"正在安全退出 HarnessDock…"')
    expect(supervisor).not.toContain('show_splash(app, "正在安全退出 HarnessDock…")')
  ...
  it('keeps hide-to-tray explicit, guarded and lifecycle-neutral', () => {
    ...
    expect(hidePrimary).toContain('.tray_available')
