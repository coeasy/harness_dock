...
describe('Tauri shell fail-open guarantees', () => {
  it('never lets optional tray, updater or native-menu setup block Harness Web startup', () => {
    ...
    expect(host).toContain('if label == "harness"')
    expect(host).toContain('if label == "main" && !has_primary_surface(app_handle)')
    expect(host).toContain('visible_webview(app, "harness") || visible_webview(app, "splash")')
    expect(host).toContain('Closing them must never terminate')
