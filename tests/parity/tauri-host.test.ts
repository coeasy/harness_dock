...
describe('Tauri v0.2 host contract', () => {
  ...
  it('publishes only after exact successful main candidate and same-SHA CI', () => {
    ...
    expect(workflow).toContain('published asset differs from exact candidate')
    expect(workflow).toContain('Immutable release asset matches')
    expect(workflow).toContain('TAURI_UPDATER_PUB_DATE="$updater_pub_date"')
  ...
  it('boots directly into the isolated Harness WebView on desktop', () => {
    ...
    expect(runtime).toContain('builder.mode(0o700)')
    expect(runtime).toContain('expected 127.0.0.1')
  ...
  it('keeps the independent Harness Shell on demand while Harness Web is mandatory', () => {
    ...
    const shellService = readFileSync(path.join(repoRoot, 'packages/plugin-harness-shell/src/index.ts'), 'utf8')
    const harnessWindow = readFileSync(path.join(repoRoot, 'apps/tauri/src-tauri/src/harness_window.rs'), 'utf8')
    const host = readFileSync(path.join(repoRoot, 'apps/tauri/src-tauri/src/lib.rs'), 'utf8')
    const web = readFileSync(path.join(repoRoot, 'apps/tauri/web/app.js'), 'utf8')
    ...
    expect(permission).toContain('commands.allow = ["control_show", "shell_settings_show", "harness_minimize", "harness_toggle_maximize", "harness_window_state", "harness_shell_close", "harness_reload_web", "harness_restart_web", "harness_safe_mode_restart", "harness_clear_quarantine_restart", "update_check", "update_install", "app_quit"]')
    ...
    expect(shellAsset).toContain("['gateway.manage', '移动设备 / Gateway']")
    ...
    expect(tray).toContain('"tray-gateway"')
    ...
    expect(harnessWindow).toContain('__harnessDockSetSurface')
    expect(web).toContain("window.addEventListener('focus'")
    ...
    expect(gatewayHost).toContain('builder.mode(0o700)')
    expect(gatewayHost).toContain('public_gateway_diagnostic')
    expect(gatewayHost).toContain('1024-65535')
    expect(gatewayHost).toContain('敏感 URL/令牌已从用户可见诊断中隐藏')
  })
