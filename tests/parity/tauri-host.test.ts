...
describe('Tauri v0.2 host contract', () => {
  ...
  it('builds Android candidates in release mode and gates native package size', () => {
    ...
    expect(candidate).toContain('cargo tauri android build --release --apk --aab')
    expect(candidate).toContain('node scripts/check-android-package.mjs')
    expect(smoke).toContain('cargo tauri android build --release --apk')
  ...
  it('keeps the Shell Settings plugin on demand while Harness Web is mandatory', () => {
    ...
    expect(permission).toContain('commands.allow = ["shell_settings_show", "harness_minimize", "harness_toggle_maximize", "harness_window_state", "harness_close"]')
    ...
    expect(settingsJs).toContain("call('runtime_start')")
