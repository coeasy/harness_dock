...
- Native startup keeps the `main` recovery window hidden and only reveals it on an explicit failure, covering upgrades, reused window state and configuration drift without adding a renderer IPC hide step.
...
The native startup coordinator owns the desktop Runtime start and keeps `main` hidden; `app.js` is only a recovery surface. This remains safe across upgrades, reused window state and configuration drift without exposing the control page as the first surface.
...
- `pnpm check:versions`
- `pnpm check:release`
- Tauri parity：桌面 `main.visible=false`、移动端 `visible=true`、capability URL pattern、权限边界、设置按需和 native recovery-window contract；
